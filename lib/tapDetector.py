...
class TapDetector:
    def __init__(self, pin_number=19, debounce_time_ms=250, 
        ...
        dan mengonversinya menjadi representasi biner.
        ...
        self._last_sequence_tap_time = 0 
        ...
        print(f"TapDetector diinisialisasi pada GPIO{self.pin_number}.")
    ...
    def _handle_tap_interrupt(self, pin_obj):
        ...
        if time.ticks_diff(current_time, self._last_tap_time) > self.debounce_time_ms:
            ...
            self._current_tap_count += 1 # Tambah hitungan ketukan fisik
            
            # --- Logika Deteksi Pola Ketukan ---
            # Jika urutan sudah lengkap, tetapi ada ketukan tambahan yang terdeteksi, reset.
            if self._full_sequence_ready and self._current_tap_count > self.total_taps_to_expect:
                print(f"[{time.ticks_ms()}] Urutan sudah lengkap, ketukan tambahan terdeteksi. Mereset.")
                self.reset_sequence()
                self._last_tap_time = current_time # Update debounce time
                return # Hentikan pemrosesan lebih lanjut untuk interupsi ini
            ...
            if self._current_tap_count <= self.total_taps_to_expect:
                if self._current_tap_count == 1:
                    ...
                    if self._current_tap_count < self.total_taps_to_expect:
                        ...
                        # Jumlah bit biner yang terkumpul saat ini adalah _current_tap_count - 1
                        ...
                        if time_diff <= self.short_tap_max_delay_ms:
                            # Jeda ini untuk ketukan ke-6 (bit terakhir)
                            self._current_binary_sequence += "."
                            print(f"[{time.ticks_ms()}] Ketukan #{self._current_tap_count} (Akhir urutan fisik).")
                            self._full_sequence_ready = True
                        else:
                            self._current_binary_sequence += "-"
                            print(f"[{time.ticks_ms()}] Jeda ketukan terakhir ({time_diff}ms) tidak pendek seperti yang diharapkan. Urutan direset.")
                            self._full_sequence_ready = True
            """                
            if self._current_tap_count <= self.total_taps_to_expect:
                if self._current_tap_count == 1:
                    # Ketukan pertama: selalu pendek (0), ini adalah bit pertama dari 6.
                    self._current_binary_sequence += "."
                    print(f"[{time.ticks_ms()}] Ketukan #1 (Awal/Pendek). Urutan biner: {self._current_binary_sequence}")
                else:
                    # Hitung jeda dari ketukan sebelumnya dalam urutan
                    time_diff = time.ticks_diff(current_time, self._last_sequence_tap_time)
                    ...
                    if self._current_tap_count < self.total_taps_to_expect:
                        ...
                        if time_diff <= self.short_tap_max_delay_ms:
                            ...
                            self.reset_sequence()
            """
            self._last_sequence_tap_time = current_time # Perbarui waktu ketukan terakhir untuk perhitungan jeda
    ...
    def is_sequence_complete(self):
        ...
        if self._full_sequence_ready:
            ...
            # Ketukan fisik ke-1 menghasilkan bit 1.
            # Ketukan fisik ke-2 sampai ke-6 menghasilkan bit 2 sampai 6.
            # Total 6 bit biner.
    ...
    def deactivate(self):
        ...
        print(f"TapDetector pada GPIO{self.pin_number} dinonaktifkan.")
    ...
    def __repr__(self):
        ...
        return (f"TapDetector(pin=GPIO{self.pin_number}, "
...
if __name__ == "__main__":
    print("Menjalankan contoh penggunaan TapDetector (Deteksi Pola Biner & Konversi)...")

    # Sesuaikan nilai parameter sesuai kebutuhan Anda.
    # Ingat: total_taps_to_expect = 7 akan menghasilkan 6 bit biner.
    detector = TapDetector(
        pin_number=19, 
        debounce_time_ms=50, 
        short_tap_max_delay_ms=300,  # Max jeda 0.5 detik untuk pendek '.'
        long_tap_min_delay_ms=301,   # Min jeda 0.501 detik untuk panjang '-'
    ...
    print(f"Ketukan pertama dianggap pendek ('.'). Jeda yang tidak valid akan mereset urutan.")
